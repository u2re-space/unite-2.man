...
- Не полагаться на VBScript/wscript или публично описанную elevation policy;
  Windows elevation и доступы задаются приватной конфигурацией окружения.
...
**Что стоит понимать?**
- Windows desk использует логическую роль `L-110`.
- Gateway/server использует логическую роль `L-200`.
- WAN simulator воспроизводит внешнюю клиентскую роль.
- Android-устройства используют логические роли `L-196`, `L-210` и `L-208`.
- Exact hosts, usernames, SSH keys/commands, elevation policy и ADB endpoints
  находятся только в `private/connectivity.md`.
